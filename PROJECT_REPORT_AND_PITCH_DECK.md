...
```mermaid
flowchart TD
    subgraph Client ["Unified Mobile Application (Expo SDK 57 / React 19)"]
        CG[Customer UI - Browse, Cart, Hamper Builder, Loyalty]
        AG[Store Admin / Kitchen Terminal - PIN 1984 Protected]
        RG[Delivery Fleet Partner - Shift Auth & Route Tracking]
    end

    subgraph RealTime ["Real-Time & Networking Layer"]
        WS[Socket.io Engine - Dynamic Room Multiplexing]
        REST[Express RESTful API - Versioned /api/v1]
    end

    subgraph Backend ["Backend Core (Node.js 22 LTS / TypeScript)"]
        AuthService[JWT & OTP Authentication Engine]
        OrderEngine[Order Processing & State Machine]
        WebhookRecon[HMAC SHA-256 Webhook Reconciliation]
        LoyaltyService[Connoisseur Rewards & Point Engine]
        DB[(Prisma ORM & SQLite Persistence)]
    end

    CG -->|Auth & Orders| REST
    AG -->|Queue Dispatch| REST
    RG -->|Delivery Status| REST
    
    CG <-->|Live Status Updates| WS
    AG <-->|Instant Kitchen Broadcasts| WS
    RG <-->|Assigned Fleet Events| WS
